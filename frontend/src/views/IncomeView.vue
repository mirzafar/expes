...
async function load() {
  ...
  try {
    const [list, cats] = await Promise.all([
      listTransactions({ type: 'income', limit: 100, ...range }),
...
<template>
  <div class="space-y-6">
    <div class="flex flex-wrap items-center justify-between gap-3">
      ...
      <button class="btn-primary" @click="ui.openAdd('income')">
        <Plus class="h-4 w-4" :stroke-width="2.4" /> Доход
      </button>
    </div>

    <StatCard label="Доходы за период" :amount="total" :icon="TrendingUp" tone="emerald" />

    <div class="grid grid-cols-1 gap-4 lg:grid-cols-5">
      <div class="lg:col-span-2">
        <CategoryDonut title="Доходы по категориям" :slices="byCat" />
      </div>
      <div class="lg:col-span-3">
        <h2 class="mb-3 font-display text-lg font-bold tracking-tight">Все доходы</h2>
        <TxTable :items="items" :loading="loading" empty-text="Нет доходов за период" @delete="onDelete" />
      </div>
    </div>
  </div>
</template>
