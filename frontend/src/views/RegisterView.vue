...
async function onSubmit() {
  ...
  try {
    ...
    error.value = e.response?.data?.detail || 'Не удалось зарегистрироваться. Попробуйте ещё раз.'
  } finally {
    loading.value = false
  }
}
  ...
  <div class="flex min-h-screen">
    ...
    <div class="relative hidden w-1/2 flex-col justify-between overflow-hidden bg-indigo-600 p-12 text-white lg:flex">
      ...
      <div class="relative">
        <h2 class="font-display text-4xl font-bold leading-tight tracking-tight">
          Начните вести<br />бюджет сегодня
        </h2>
        <p class="mt-4 max-w-md text-indigo-100">
          Бесплатно. Пара минут на регистрацию — и вы видите полную картину своих финансов.
        </p>
    ...
    <div class="flex w-full items-center justify-center px-6 py-12 lg:w-1/2">
      <div class="w-full max-w-sm">
        <div class="mb-8">
          <h1 class="font-display text-2xl font-bold tracking-tight">Создать аккаунт</h1>
          <p class="mt-1.5 text-sm text-slate-500">Заполните данные, чтобы начать</p>
        </div>

        <form class="space-y-4" @submit.prevent="onSubmit">
          <div>
            <label class="label">Имя</label>
            <input v-model="name" type="text" class="input" required autocomplete="name" placeholder="Иван Иванов" />
          </div>
          <div>
          ...
          <div>
            ...
            <input v-model="password" type="password" class="input" required minlength="6" autocomplete="new-password" placeholder="минимум 6 символов" />
          </div>
        ...
        <p class="mt-6 text-center text-sm text-slate-500">
          Уже есть аккаунт?
          <RouterLink to="/login" class="font-semibold text-indigo-600 hover:text-indigo-700">Войти</RouterLink>
        </p>
      </div>
    </div>
  </div>
